...
/* ---- 일자별 입고 기록 바코드 인쇄 (2차검증용) ---- */
// CSV 파일 자체에는 그림을 넣을 수 없어서, 인쇄용 화면을 따로 띄운다.
// 각 줄에 스캔 가능한 바코드가 같이 찍혀 나오므로 종이를 들고 PDA로 다시 검증할 수 있다.
function openDayBarcodePrint() {
  const rows = currentHistoryDayLogs();
  if (rows.length === 0) { toast('선택한 날짜에 인쇄할 입고 기록이 없습니다.', true); return; }
  const pad = n => String(n).padStart(2, '0');
  const batchLabel = state.ui.historyBatch === HISTORY_BATCH_NONE ? '미지정'
    : state.ui.historyBatch ? state.ui.historyBatch : '전체';
  const title = `입고기록 ${state.ui.historyYear}-${pad(state.ui.historyMonth)}-${pad(state.ui.historyDay)} · 차수/업체 ${batchLabel}`;

  const body = rows.map((l, i) => `
    <tr>
      <td class="num">${i + 1}</td>
      <td class="bc">${barcodeSvg(l.barcode, { moduleW: 2, maxWidth: 230, height: 42 })}</td>
...
<style>
  *{box-sizing:border-box;}
  body{ font-family: system-ui,-apple-system,"Malgun Gothic",sans-serif; margin:16px; color:#000; }
  ...
  td.bc{ width:244px; text-align:center; padding:3px 6px; }
  td.bc svg{ display:block; margin:0 auto; max-width:100%; height:auto; }
  td.qty{ width:56px; text-align:right; font-variant-numeric:tabular-nums; font-weight:700; }
  td.loc{ width:110px; font-family:ui-monospace,Menlo,monospace; font-weight:700; font-size:12px; }
  td.chk{ width:52px; }
...
<h1>${esc(title)}</h1>
<p class="sub">총 ${rows.length}건 · 바코드를 스캔해서 실제 적치 위치와 대조하세요. 확인란에 체크하시면 됩니다.</p>
<table>
  <thead><tr><th>#</th><th>바코드</th><th>상품명</th><th>수량</th><th>로케이션</th><th>확인</th></tr></thead>
  <tbody>${body}</tbody>
</table>
</body></html>`;
...
function bindGlobalEvents() {
  ...
  const btnDayPrint = document.getElementById('btnDayBarcodePrint');
  if (btnDayPrint) btnDayPrint.addEventListener('click', openDayBarcodePrint);
