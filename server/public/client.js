...
function barcodeSvg(code, opts) {
  ...
  if (o.maxWidth && totalModules * mw > o.maxWidth) {
...
function buildLabelSheetHtml(rows, title) {
  const cells = rows.map(r => {
    ...
    const locText = r.partnerLoc ? `${r.loc} + ${r.partnerLoc.slice(-2)}` : r.loc;
    ...
    <div class="label">
      <table>
        ...
        <tr class="r-scan">
          <th>SCAN</th>
          <td colspan="3"><div class="scanbox">${barcodeSvg(r.barcode, { moduleW: 3, maxWidth: 560, height: 96, hideLabel: true })}</div></td>
        </tr>
        <tr class="r-foot">
          <th>수량</th>
          <td class="qty">${esc(r.qty)}</td>
          <th class="th-loc">로케이션</th>
          <td class="loc">${esc(locText)}</td>
        </tr>
        <tr class="r-date">
          <th>입고일</th>
          <td colspan="3" class="date">${esc(dateText)}</td>
        </tr>
      </table>
    </div>`;
  }).join('');
...
<style>
  ...
  *{ box-sizing:border-box; }
  body{ font-family: system-ui,-apple-system,"Malgun Gothic",sans-serif; margin:0; padding:6mm; color:#000; background:#fff; }
  .noprint{ margin-bottom:8mm; }
  .noprint button{ font-size:14px; padding:8px 16px; cursor:pointer; margin-right:8px; }
  .noprint .cnt{ font-size:13px; color:#444; }

  /* A4 한 장에 2개 — 최대한 크게 뽑아 높은 단에서도 읽히게 */
  .label{
    width:100%; height:138mm; margin:0 0 3mm; padding:0;
    page-break-inside:avoid; break-inside:avoid;
  }
  .label:nth-child(2n){ page-break-after:always; break-after:page; }
  .label table{ width:100%; height:100%; border-collapse:collapse; table-layout:fixed; }
  .label th, .label td{ border:3px solid #000; padding:2mm 3mm; }
  .label th{
    width:26mm; font-size:19px; font-weight:800; text-align:center;
    background:#fff; letter-spacing:1px; white-space:nowrap;
  }
  .label th.th-loc{ width:28mm; }
  ...
  .r-code{ height:32mm; }
  .bignum{
    font-size:62px; font-weight:900; text-align:center; letter-spacing:2px;
  ...
  .r-foot{ height:26mm; }
  .r-foot .qty{ font-size:54px; font-weight:900; text-align:center; width:34mm; }
  .r-foot .loc{
  ...
  .r-date{ height:16mm; }
  .r-date .date{ font-size:26px; font-weight:800; text-align:center; letter-spacing:1px; }

  @media print{ .noprint{ display:none; } body{ padding:0; } }
</style></head><body>
<div class="noprint">
  <button onclick="window.print()">🖨 인식표 인쇄</button>
  <span class="cnt">${rows.length}장 · A4 한 장에 2개씩 인쇄됩니다. 높은 단에서도 보이도록 바코드 숫자와 로케이션을 크게 넣었습니다.</span>
</div>
${cells}
</body></html>`;
}
