...
const state = {
  ...
  ui: {
    ...
    logSearch: '', logVerify: '',
    logDate: '', logBatch: '',   // 할당기록·검증 탭의 날짜 / 차수·업체명 필터 ('' = 전체)
    theme: 'light',
...
function connectRealtime() {
  ...
  socket.on('state:init', shared => {
    ...
    // 이전 버전에서 쌓인 기록을 «할당 실행 이력»으로 한 번 옮겨 담는다(있을 때만).
    const filled = backfillInboundHistory();
    renderAll();
    if (filled > 0) syncToServer(true);   // 채워넣은 결과를 서버에도 저장
...
function runAllocationForItem(barcode, totalQty, priorityZoneStr, batchLabel, skipLocs) {
  ...
  while (remaining > 0) {
    ...
    if (mergeLoc) {
      ...
      const mergeLogId = uid(), mergeTs = Date.now();
      placements.push({ loc: mergeLoc.loc, partnerLoc: null, qty: addQty, is2plt: false, merged: true, logId: mergeLogId, ts: mergeTs });
      state.log.push({
        id: mergeLogId, ts: mergeTs, barcode, name: (product && product.name) || '',
    ...
    if (found) {
      ...
      existingZones = Array.from(new Set(existingFullLocs.map(l => l.slice(0, 3))));
      const ts = Date.now(), placeLogId = uid();
      placements.push({ loc: found.mainLoc, partnerLoc: found.partnerLoc || null, qty: currentPutQty, is2plt: is2Plt, logId: placeLogId, ts });
      state.log.push({
        id: placeLogId, ts, barcode, name: (product && product.name) || '',
...
/* ---- log ---- */
const LOG_BATCH_NONE = '__none__';   // "차수/업체명 미지정"을 가리키는 내부 값

// 기록 한 건의 날짜를 'YYYY-MM-DD' 문자열로 (날짜 필터 비교용)
function logDateKey(ts) {
...
function filteredLog() {
  ...
  const day = state.ui.logDate;
  const batch = state.ui.logBatch;
  return state.log.filter(l => {
    ...
    if (day && logDateKey(l.ts) !== day) return false;
    if (batch === LOG_BATCH_NONE) { if (l.batch) return false; }
    else if (batch && l.batch !== batch) return false;
    if (q && !(l.barcode.toLowerCase().includes(q) || l.loc.toLowerCase().includes(q))) return false;
    return true;
  });
}

// 날짜 / 차수·업체명 선택지를 실제 기록에 있는 값들로 다시 채운다.
// 차수 목록은 «선택된 날짜»에 존재하는 값만 보여줘서, 없는 조합을 고르는 일이 없게 한다.
function renderLogFilters() {
  const dateSel = document.getElementById('logDateFilter');
  const batchSel = document.getElementById('logBatchFilter');
  if (!dateSel || !batchSel) return;

  const dates = Array.from(new Set(state.log.map(l => logDateKey(l.ts)))).sort().reverse();
  if (state.ui.logDate && !dates.includes(state.ui.logDate)) state.ui.logDate = '';
  dateSel.innerHTML = ['<option value="">전체 날짜</option>']
    .concat(dates.map(d => `<option value="${d}">${d}</option>`)).join('');
  dateSel.value = state.ui.logDate;

  const scoped = state.log.filter(l => !state.ui.logDate || logDateKey(l.ts) === state.ui.logDate);
  const labels = Array.from(new Set(scoped.map(l => l.batch || ''))).filter(Boolean).sort();
  const hasUnlabeled = scoped.some(l => !l.batch);
  const opts = ['<option value="">전체 차수/업체</option>'];
  if (hasUnlabeled && labels.length) opts.push(`<option value="${LOG_BATCH_NONE}">미지정</option>`);
  labels.forEach(b => opts.push(`<option value="${esc(b)}">${esc(b)}</option>`));
  batchSel.innerHTML = opts.join('');
  const valid = [''].concat(hasUnlabeled ? [LOG_BATCH_NONE] : []).concat(labels);
  if (!valid.includes(state.ui.logBatch)) state.ui.logBatch = '';
  batchSel.value = state.ui.logBatch;
}
...
// 남는 수량을 다른 자리로 보냈을 때, 그 배치도 «할당 실행 이력»에 같이 남긴다.
// 이게 없으면 12개를 5개로 줄이고 7개를 옮겼을 때 일자별 입고 기록에 5개만 남아
// 엑셀에 붙여넣는 총량이 실제 입고량과 달라진다.
function appendPlacementsToExecutionHistory(sourceLogId, barcode, name, placements, ts, batch) {
  if (!placements || placements.length === 0) return;
  let targetResult = null;
  (state.executionHistory || []).forEach(b => (b.results || []).forEach(r => {
    if (r.barcode !== barcode) return;
    (r.placements || []).forEach(pl => {
      if (pl.logId && pl.logId === sourceLogId) targetResult = r;
    });
  }));
  if (targetResult) {
    // 원래 그 입고 건의 일부이므로 같은 묶음에 붙인다
    placements.forEach(pl => targetResult.placements.push(pl));
    return;
  }
  // 원본 묶음을 못 찾으면(옛 데이터 등) 새 묶음으로 남긴다
  state.executionHistory.unshift({
    id: uid(), ts: ts || Date.now(), batch: batch || '',
    results: [{ barcode, name: name || '', requestedQty: 0, allocatedQty: 0, remaining: 0, placements }]
  });
}

// 같은 상품이 이미 «덜 찬 상태»로 들어가 있는 자리들(=자투리 자리) 목록.
// 남는 수량을 새 자리에 또 만들지 말고 기존 자투리부터 채우고 싶을 때 쓴다.
function partialLocationsFor(barcode, excludeLocs) {
  const product = findProduct(barcode);
  const pltQty = Number(product && product.pltQty) || 0;
  if (!(pltQty > 0)) return [];
  const skip = new Set(excludeLocs || []);
  return state.locations
    .filter(l => {
      if (skip.has(l.loc)) return false;
      if (l.barcode !== barcode) return false;
      const q = Number(l.qty);
      return Number.isFinite(q) && q > 0 && q < pltQty;
    })
    .map(l => ({ loc: l.loc, qty: Number(l.qty), room: pltQty - Number(l.qty) }))
    .sort((a, b) => a.room - b.room || naturalCompare(a.loc, b.loc));
}

// 할당기록에서 로케이션·수량을 고치면, «할당 실행 이력»에 들어 있는 같은 배치 기록도 같이 고친다.
// (엑셀에 붙여넣는 «일자별 입고 기록»이 실제 적치 내용과 어긋나지 않도록)
// 삭제는 반영하지 않는다 — 할당기록에서만 지워지고 입고 기록은 그대로 남는다.
function applyEditToExecutionHistory(logId, barcode, oldLoc, newLoc, newQty) {
  (state.executionHistory || []).forEach(batchEntry => {
    (batchEntry.results || []).forEach(r => {
      if (r.barcode !== barcode) return;
      (r.placements || []).forEach(pl => {
        const hit = pl.logId ? (pl.logId === logId) : (pl.loc === oldLoc);
        if (!hit) return;
        pl.loc = newLoc;
        pl.qty = newQty;
      });
...
function openLogLocEditor(id) {
  ...
  body.innerHTML = `
    ...
    <div id="dwLeftoverBox" class="leftover-box" style="display:none;">
      ...
      <label class="leftover-opt"><input type="radio" name="dwLeftover" value="merge" id="dwLeftoverMergeRadio"> 덜 찬 자리(자투리)에 채우기</label>
      <select id="dwLeftoverMergeLoc" disabled></select>
  ...
  const leftoverMergeSel = document.getElementById('dwLeftoverMergeLoc');
  const leftoverMergeRadio = document.getElementById('dwLeftoverMergeRadio');
  const leftoverMode = () => (body.querySelector('input[name="dwLeftover"]:checked') || {}).value || 'auto';

  // «덜 찬 자리» 후보 채우기 — 지금 고치고 있는 자리는 후보에서 뺀다
  function syncMergeOptions() {
    const curLoc = input.value.trim().toUpperCase();
    const cands = partialLocationsFor(entry.barcode, [curLoc, entry.loc]);
    if (cands.length === 0) {
      leftoverMergeSel.innerHTML = '<option value="">덜 찬 자리가 없습니다</option>';
      leftoverMergeSel.disabled = true;
      leftoverMergeRadio.disabled = true;
      leftoverMergeRadio.parentElement.style.opacity = '.45';
      if (leftoverMode() === 'merge') body.querySelector('input[name="dwLeftover"][value="auto"]').checked = true;
    } else {
      leftoverMergeSel.innerHTML = cands.map(c =>
        `<option value="${esc(c.loc)}">${esc(c.loc)} · 현재 ${c.qty}개 (${c.room}개 더 들어감)</option>`).join('');
      leftoverMergeRadio.disabled = false;
      leftoverMergeRadio.parentElement.style.opacity = '';
      leftoverMergeSel.disabled = leftoverMode() !== 'merge';
    }
  }
  ...
  body.querySelectorAll('input[name="dwLeftover"]').forEach(r => {
    r.addEventListener('change', () => {
      const mode = leftoverMode();
      leftoverLocInput.disabled = mode !== 'manual';
      leftoverMergeSel.disabled = mode !== 'merge';
      if (mode === 'manual') leftoverLocInput.focus();
      if (mode === 'merge') leftoverMergeSel.focus();
    });
  });
  input.addEventListener('input', syncMergeOptions);
  syncMergeOptions();
  syncLeftoverBox();
  ...
  document.getElementById('dwLogLocSave').onclick = () => {
    ...
    if (leftover > 0 && mode === 'merge') {
      mergeLoc = leftoverMergeSel.value;
      if (!mergeLoc) { toast('채워 넣을 자투리 자리를 선택해주세요.', true); return; }
    }

    const prevLoc = entry.loc;
    releaseLogEntryStock(entry);   // 원래 자리에서 이 기록만큼 내리고
    entry.loc = newLoc;
    entry.qty = newQty;
    occupyLogEntryStock(entry);    // 새 자리(또는 같은 자리·새 수량)에 다시 올린다
    applyEditToExecutionHistory(entry.id, entry.barcode, prevLoc, newLoc, newQty);

    // 남는 수량을 옮긴 것도 «실행 이력»에 같이 남겨야 입고 기록 총량이 맞는다
    const extraPlacements = [];
    const pushSplit = (loc, qty, merged) => {
      const split = {
        id: uid(), ts: Date.now(), barcode: entry.barcode, name: entry.name || '',
        qty, loc, is2plt: !!entry.is2plt, verified: null,
        batch: entry.batch || '', merged: !!merged, note: '수량 정정으로 분리'
      };
      state.log.push(split);
      occupyLogEntryStock(split);
      extraPlacements.push({ loc, partnerLoc: null, qty, is2plt: !!entry.is2plt, merged: !!merged, logId: split.id, ts: split.ts });
      return split;
    };

    let extra = '';
    if (leftover > 0 && mode === 'manual') {
      pushSplit(leftoverLoc, leftover, false);
      extra = ` 남은 ${leftover}개는 ${leftoverLoc} 에 넣었습니다.`;
    } else if (leftover > 0 && mode === 'merge') {
      // 고른 자투리 자리에 들어갈 만큼만 채우고, 남으면 나머지는 자동 배치
      const cand = partialLocationsFor(entry.barcode, [newLoc]).find(c => c.loc === mergeLoc);
      const room = cand ? cand.room : leftover;
      const put = Math.max(0, Math.min(leftover, room));
      if (put > 0) pushSplit(mergeLoc, put, true);
      const rest = leftover - put;
      extra = put > 0 ? ` 남은 ${leftover}개 중 ${put}개를 ${mergeLoc} 에 채웠습니다.` : '';
      if (rest > 0) {
        const res = runAllocationForItem(entry.barcode, rest, '', entry.batch || '', [newLoc, entry.loc, mergeLoc]);
        (res.placements || []).forEach(pl => extraPlacements.push(pl));
        extra += res.allocatedQty > 0
          ? ` 나머지 ${rest}개는 ${res.placements.map(pl => pl.loc).join(', ')} 에 배치했습니다.`
          : ` 나머지 ${rest}개는 빈 자리를 찾지 못했습니다.`;
      }
      ...
      (res.placements || []).forEach(pl => extraPlacements.push(pl));
      const locs = res.placements.map(pl => pl.loc);
      extra = res.allocatedQty > 0
    ...
    appendPlacementsToExecutionHistory(entry.id, entry.barcode, entry.name, extraPlacements, entry.ts, entry.batch);

    markDirty();
    closeDrawer(); renderLog(); renderLocations(); renderDashboard(); renderHistoryDayNav();
...
function deleteLogEntry(id) {
  ...
  const msg = `${entry.loc} 에 적치한 ${entry.barcode} ${entry.qty}개 기록을 삭제할까요?\n\n`
    ...
    + `«할당 실행 이력»의 일자별 입고 기록에는 그대로 남습니다.\n`
    + `이 동작은 되돌릴 수 없습니다.`;
...
function bindGlobalEvents() {
  ...
  document.getElementById('btnRunAllocation').addEventListener('click', () => {
    ...
    // 실행 이력은 자동으로 지우지 않는다. «설정 · 백업 → 전체 초기화»로만 비워진다.
  ...
  document.getElementById('btnDayCsvExport').addEventListener('click', () => {
    ...
    const header = csvRow(INBOUND_COLUMNS) + '\n';
    const body = rows.map(l => csvRow(inboundFormRow(l))).join('\n');
    const pad = n => String(n).padStart(2, '0');
  ...
  const logDateSel = document.getElementById('logDateFilter');
  if (logDateSel) logDateSel.addEventListener('change', e => {
    state.ui.logDate = e.target.value; state.ui.logBatch = ''; state.ui.logPage = 1; renderLog();
  });
  const logBatchSel = document.getElementById('logBatchFilter');
  if (logBatchSel) logBatchSel.addEventListener('change', e => {
    state.ui.logBatch = e.target.value; state.ui.logPage = 1; renderLog();
  });
  ...
  const btnDayHistDel = document.getElementById('btnDayHistoryDelete');
  if (btnDayHistDel) btnDayHistDel.addEventListener('click', deleteShownInboundHistory);
  ...
  document.getElementById('btnLogClear').addEventListener('click', () => {
    // 날짜·차수·검색 필터가 걸려 있으면 «화면에 보이는 것»만 지운다.
    // 필터를 걸어둔 채 눌렀다가 안 보이는 기록까지 날리는 사고를 막기 위함.
    const shown = filteredLog();
    const all = state.log.length;
    if (shown.length === 0) { toast('삭제할 기록이 없습니다.', true); return; }
    const isFiltered = shown.length < all;
    const msg = isFiltered
      ? `지금 화면에 보이는 ${shown.length}건만 삭제할까요? (전체 ${all}건 중)\n\n`
        + `필터를 해제하면 나머지 ${all - shown.length}건은 그대로 남아 있습니다.\n`
        + `«할당 실행 이력»의 일자별 입고 기록에는 영향이 없습니다.\n`
        + `이 동작은 되돌릴 수 없습니다.`
      : `할당기록 ${all}건을 전부 삭제할까요?\n\n`
        + `«할당 실행 이력»의 일자별 입고 기록에는 영향이 없습니다.\n`
        + `이 동작은 되돌릴 수 없습니다.`;
    if (!confirm(msg)) return;
    const kill = new Set(shown.map(l => l.id));
    state.log = state.log.filter(l => !kill.has(l.id));
    markDirty(); renderLog(); renderDashboard();
    toast(`할당기록 ${shown.length}건을 삭제했습니다.`);
  });
...
/* ---- 일자별 입고 기록 (엑셀 붙여넣기용) ---- */
// 예전 버전에서 만들어진 데이터 호환용.
// «할당 실행 이력»에 아직 들어 있지 않은 할당기록이 있으면, 날짜+차수 단위로 묶어서 실행 이력에 채워 넣는다.
// 이렇게 해두면 이번 변경 이전에 쌓인 기록도 «일자별 입고 기록»에 그대로 남고,
// 할당기록에서 지워도 사라지지 않는다. 이미 들어 있는 건 건드리지 않으므로 여러 번 실행해도 안전하다.
function backfillInboundHistory() {
  const coveredIds = new Set();
  const coveredKeys = new Set();
  const keyOf = (barcode, loc, qty, ts) => {
    const p = dateParts(ts);
    return `${barcode}|${loc}|${qty}|${p.y}-${p.m}-${p.d}`;
  };
  (state.executionHistory || []).forEach(b => (b.results || []).forEach(r => (r.placements || []).forEach(pl => {
    if (pl.logId) coveredIds.add(pl.logId);
    coveredKeys.add(keyOf(r.barcode, pl.loc, pl.qty, pl.ts || b.ts));
  })));

  const missing = (state.log || []).filter(l => {
    if (coveredIds.has(l.id)) return false;
    // logId가 없던 옛 실행 이력과 중복 생성되지 않도록 내용으로도 한 번 더 확인
    return !coveredKeys.has(keyOf(l.barcode, l.loc, l.qty, l.ts));
  });
  ...
  groups.forEach(entries => {
    ...
    entries.forEach(l => {
      if (!byBarcode.has(l.barcode)) {
        byBarcode.set(l.barcode, { barcode: l.barcode, name: l.name || '', requestedQty: 0, allocatedQty: 0, remaining: 0, placements: [] });
      }
      const r = byBarcode.get(l.barcode);
      r.placements.push({ loc: l.loc, partnerLoc: null, qty: l.qty, is2plt: !!l.is2plt, merged: !!l.merged, logId: l.id, ts: l.ts });
      const q = Number(l.qty) || 0;
      r.allocatedQty += q; r.requestedQty += q;
    });
    added.push({
      id: uid(),
      ts: Math.min(...entries.map(e => e.ts)),
      batch: entries[0].batch || '',
      results: Array.from(byBarcode.values()),
      backfilled: true
    });
  });
  state.executionHistory = added.concat(state.executionHistory || []).sort((a, b) => b.ts - a.ts);
  return missing.length;
}

// ===== 일자별 입고 기록의 원본 =====
// «할당 실행 이력»(state.executionHistory)에서 만든다. «할당기록 · 검증»의 state.log 와는 별개라서,
// 거기서 기록을 지워도 여기 입고 기록은 남는다. 반대로 로케이션·수량을 «수정»하면
// applyEditToExecutionHistory()가 이쪽 값도 같이 고쳐줘서 엑셀에 붙여넣는 기록이 실제와 어긋나지 않는다.
function executionInboundRows() {
  const rows = [];
  (state.executionHistory || []).forEach(batchEntry => {
    (batchEntry.results || []).forEach(r => {
      (r.placements || []).forEach(pl => {
        rows.push({
          id: pl.logId || `${batchEntry.id}:${r.barcode}:${pl.loc}`,
          ts: pl.ts || batchEntry.ts,
          barcode: r.barcode,
          name: r.name || '',
          qty: pl.qty,
          loc: pl.loc,
...
function historyAvailableYears() {
  return Array.from(new Set(executionInboundRows().map(l => dateParts(l.ts).y))).sort((a, b) => b - a);
}
function historyAvailableMonths(year) {
  return Array.from(new Set(executionInboundRows().filter(l => dateParts(l.ts).y === year).map(l => dateParts(l.ts).m))).sort((a, b) => a - b);
}
function historyAvailableDays(year, month) {
  return Array.from(new Set(executionInboundRows().filter(l => {
    const p = dateParts(l.ts);
    return p.y === year && p.m === month;
  }).map(l => dateParts(l.ts).d))).sort((a, b) => a - b);
}
// 선택한 날짜의 기록 전체(차수/업체명 필터 적용 전) — 그 날짜에 실제로 쓰인 차수/업체명 목록을 구하는 데 사용.
function logsForSelectedDay() {
  ...
  if (!y || !m || !d) return [];
  return executionInboundRows().filter(l => {
    const p = dateParts(l.ts);
...
// 지금 «일자별 입고 기록»에 보이는 조건(날짜 + 차수/업체명)의 입고 기록을 실행 이력에서 지운다.
// 실제 로케이션 재고는 건드리지 않는다 — 이건 «기록»을 지우는 것이지 물건을 빼는 게 아니다.
function deleteShownInboundHistory() {
  const rows = currentHistoryDayLogs();
  if (rows.length === 0) { toast('삭제할 입고 기록이 없습니다.', true); return; }
  const pad = v => String(v).padStart(2, '0');
  const dateText = `${state.ui.historyYear}-${pad(state.ui.historyMonth)}-${pad(state.ui.historyDay)}`;
  const batchText = state.ui.historyBatch === HISTORY_BATCH_NONE ? '차수/업체명 미지정'
    : state.ui.historyBatch ? state.ui.historyBatch : '이 날짜 전체';
  const msg = `${dateText} · ${batchText}\n입고 기록 ${rows.length}건을 삭제할까요?\n\n`
    + `«할당 실행 이력»에서만 지워집니다. 실제 로케이션 재고와 «할당기록 · 검증» 목록은 그대로입니다.\n`
    + `이 동작은 되돌릴 수 없습니다.`;
  if (!confirm(msg)) return;
  ...
  const killKeys = new Set(rows.map(r => keyOf(r.barcode, r.loc, r.qty, r.ts)));

  (state.executionHistory || []).forEach(b => {
    (b.results || []).forEach(r => {
      r.placements = (r.placements || []).filter(pl => {
        const id = pl.logId || `${b.id}:${r.barcode}:${pl.loc}`;
        if (kill.has(id)) return false;
        // logId가 없던 옛 기록은 내용으로 대조
        if (!pl.logId && killKeys.has(keyOf(r.barcode, pl.loc, pl.qty, pl.ts || b.ts))) return false;
        return true;
      });
    });
    b.results = (b.results || []).filter(r => (r.placements || []).length > 0);
  });
  state.executionHistory = (state.executionHistory || []).filter(b => (b.results || []).length > 0);

  markDirty();
  renderHistoryDayNav();
  toast(`입고 기록 ${rows.length}건을 삭제했습니다.`);
}

// ===== 입고 양식(엑셀 붙여넣기용) 열 정의 =====
// 창고에서 쓰는 입고 엑셀 양식의 열 순서를 한 곳에서만 관리한다.
// 화면 표, 클립보드 복사(TSV), CSV 내보내기가 모두 이 정의를 쓰기 때문에
// 양식이 바뀌면 여기 세 상수와 inboundFormRow()만 고치면 전부 따라간다.
const INBOUND_COLUMNS = ['박스번호', '바코드', '정상수량', '불량수량', '정상다중로케이션', '불량다중로케이션', '유통기한', 'LOT', '품명'];
const INBOUND_BAD_LOC = '00-00-00-00';      // 불량다중로케이션 기본값 (불량은 시스템에서 관리하지 않음)
const INBOUND_DEFAULT_EXPIRY = '9999-12-31'; // 유통기한 기본값 (양식의 기본 표기)

// 입고 기록 1건 -> 입고 양식 한 줄.
// 박스번호·LOT은 현재 시스템에서 관리하지 않아 빈 칸으로 두고,
// 불량수량/불량다중로케이션은 항상 0 / 00-00-00-00 으로 통일한다.
function inboundFormRow(l) {
  return ['', l.barcode, l.qty, 0, l.loc, INBOUND_BAD_LOC, INBOUND_DEFAULT_EXPIRY, '', l.name || ''];
}
...
function renderHistoryDayLog() {
  ...
  if (rows.length === 0) {
    body.innerHTML = `<tr><td colspan="${INBOUND_COLUMNS.length}" class="empty-state">선택한 날짜의 입고 기록이 없습니다.</td></tr>`;
    return;
  }
  body.innerHTML = rows.map(l => `
    <tr>
      ...
      <td class="mono">${esc(l.loc)}</td>
      <td class="mono muted">${INBOUND_BAD_LOC}</td>
      <td class="mono muted">${INBOUND_DEFAULT_EXPIRY}</td>
      <td class="mono muted">&nbsp;</td>
      <td>${esc(l.name || '')}</td>
    </tr>`).join('');
}

// 입고 양식과 동일한 열 순서의 TSV.
// 엑셀에 그대로 붙여넣을 수 있도록 헤더 없이 데이터 행만 만든다.
function buildDayTsv() {
  return currentHistoryDayLogs()
    .map(l => inboundFormRow(l).join('\t'))
    .join('\n');
}
