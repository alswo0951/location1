...
if (fs.existsSync(STATE_FILE)) {
  try {
    ...
    console.log('[state] 기존 저장 데이터를 불러왔습니다:', STATE_FILE);
  } catch (err) {
    console.error('[state] 저장된 state.json을 읽지 못해 시드 데이터로 시작합니다:', err.message);
    sharedState = freshState();
  }
} else {
  sharedState = freshState();
  console.log('[state] 저장된 데이터가 없어 시드 데이터로 시작합니다.');
}
...
function persistState() {
  ...
  saveTimer = setTimeout(() => {
    ...
    fs.writeFile(STATE_FILE, JSON.stringify(sharedState), err => {
      if (err) console.error('[state] 저장 실패:', err.message);
    });
  }, 300);
}
